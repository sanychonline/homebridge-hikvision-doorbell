...
const { preconnectRtsp } = require("./hikvision-rtsp-preconnect");
...
class HikvisionCameraStreamingDelegate {
  ...
  setLiveStreamSink(sink) {
    this.liveStreamSink = typeof sink === "function" ? sink : null;
  }
  ...
  async prepareStream(request, callback) {
    ...
    preconnectRtsp(this.config, this.platform.log, request.sessionID);
    this.platform.log.info(`Prepared Hikvision stream session ${request.sessionID}: target=${sessionInfo.address}:${sessionInfo.videoPort}`);
  ...
  async handleStreamRequest(request, callback) {
    this.platform.log.info(`Hikvision stream request: type=${request.type}, session=${request.sessionID}`);
    ...
    if (request.type === "start") {
      try {
        ...
        this.platform.log.error(`Failed to start Hikvision camera stream: ${error.message}`);
        callback(error);
      }
      return;
    }
    ...
    if (request.type === "reconfigure") {
      ...
      this.platform.log.info(`Ignoring Hikvision stream reconfigure for active session ${request.sessionID}`);
      callback();
      return;
    }

    if (request.type === "stop") {
      this.platform.log.info(`Stopping Hikvision stream session ${request.sessionID}: reason=stop`);
  ...
  async handleSnapshotRequest(request, callback) {
    try {
      this.platform.log.debug(`Local Hikvision snapshot requested for ${this.config.name || this.config.did}`);
      const snapshot = await this.getLocalSnapshot(request);
      callback(undefined, snapshot);
    } catch (error) {
      this.platform.log.warn(`Failed to refresh Hikvision snapshot for ${this.config.name || this.config.did}: ${error.message}`);
  ...
  async startStream(request) {
    ...
    if (activeStreams >= this.maxStreams) {
      throw new Error(`Maximum concurrent Hikvision streams reached (${this.maxStreams}).`);
    }
    ...
    try {
      ...
      throw new Error("RTSP stream is required. Configure ip, username and password, or set rtspUrl for a custom stream path.");
    } finally {
      this.liveStreamStarting = Math.max(0, this.liveStreamStarting - 1);
    }
  }

  notifyLiveStreamStarted(request, purpose) {
    try {
      this.liveStreamSink?.({
      ...
      this.platform.log.warn(`live.stream.sink.error camera=${this.config.name || this.config.did} error=${error.message}`);
    }
  }
  ...
  spawnFfmpegFromMissReader(request, session, reader, attempt, streamPurpose = "live") {
    ...
    this.platform.log.info(`Starting native Hikvision MISS stream attempt ${attempt} for ${this.config.name || this.config.did}`);
    ...
    const onPacket = (packet) => {
      if (packet.codec === "h264" || packet.codec === "h265") {
        ...
        if (!videoPipePrimed && packet.codec === "h264") {
          ...
          if (videoBackpressureDrops === 1 || videoBackpressureDrops % 25 === 0) {
            this.platform.log.warn(`Dropping delayed Hikvision video packets for ${this.config.name || this.config.did}: session=${request.sessionID}, buffered=${videoPipe.writableLength}, drops=${videoBackpressureDrops}`);
          }
        } else {
      ...
      if (packet.codec === "pcma") {
        ...
        if (!firstAudioLogged) {
          ...
          this.platform.log.info(`Native Hikvision MISS first audio for ${this.config.name || this.config.did}: session=${request.sessionID}, codec=${packet.codec}, packetSampleRate=${packet.sampleRate || "unknown"}, ffmpegSampleRate=${missAudioSampleRate}`);
        }
        if (audioPacer) {
          ...
          if (audioBackpressureDrops === 1 || audioBackpressureDrops % 25 === 0) {
            this.platform.log.warn(`Dropping delayed Hikvision audio packets for ${this.config.name || this.config.did}: session=${request.sessionID}, buffered=${audioPipe.writableLength}, drops=${audioBackpressureDrops}`);
          }
        } else {
    ...
    const onReaderError = (error) => {
      this.platform.log.warn(`Native Hikvision MISS packet reader failed for ${this.config.name || this.config.did}: ${error.message}`);
      proc.kill("SIGTERM");
    };
    ...
    proc.on("exit", (code, signal) => {
      ...
      if (code && code !== 0 && ffmpegStderr.length && !proc.expectedStopReason) {
        ...
        this.platform.log.warn(`Native Hikvision MISS ffmpeg failed for ${this.config.name || this.config.did}: code=${code}, stderr=${ffmpegStderr.join(" | ")}`);
      } else if (code && code !== 0 && proc.expectedStopReason) {
        this.platform.log.debug(`Native Hikvision MISS ffmpeg exited after expected stop for ${this.config.name || this.config.did}: reason=${proc.expectedStopReason}, code=${code}`);
      }
      ...
      this.platform.log.info(`Native Hikvision MISS stream ended for ${this.config.name || this.config.did}: session=${request.sessionID}, code=${code}, signal=${signal}, videoPackets=${videoPackets}, audioPackets=${audioPackets}, videoBackpressureDrops=${videoBackpressureDrops}, audioBackpressureDrops=${audioBackpressureDrops}`);
  ...
  spawnFfmpegFromRtspStream(request, session, rtspUrl, streamPurpose = "live") {
    ...
    const includeAudio = Boolean(session.audioPort && this.config.audio !== false);
    const homeKitAudioSampleRate = normalizeHomeKitAudioSampleRate(this.config.homeKitAudioSampleRate);
    const rtspAudioSampleRate = normalizeDeviceAudioSampleRate(this.config.rtspAudioSampleRate);
    ...
    proc.on("error", (error) => {
      this.metrics?.increment("homekit_live_ffmpeg_errors_total");
      this.platform.log.error(`Failed to launch RTSP ffmpeg for ${this.config.name || this.config.did}: ${error.message}`);
    });
    ...
    proc.stderr.on("data", (chunk) => {
      ...
      diagnosticLog(this.config, `stream-rtsp ffmpeg session=${request.sessionID} ${ffmpegLine}`);
      if (this.config.ffmpegDebug) {
        this.platform.log.info(`[ffmpeg rtsp] ${ffmpegLine}`);
      }
    });
    ...
    proc.on("exit", (code, signal) => {
      this.platform.log.debug(`RTSP ffmpeg exited for ${this.config.name || this.config.did}: code=${code}, signal=${signal}`);
      diagnosticLog(this.config, `stream-exit session=${request.sessionID} code=${code} signal=${signal} source=rtsp videoPackets=${videoPackets} audioPackets=${audioPackets}`);
      if (code && code !== 0 && ffmpegStderr.length && !proc.expectedStopReason) {
        this.metrics?.increment("homekit_live_ffmpeg_failures_total");
        this.platform.log.warn(`RTSP ffmpeg failed for ${this.config.name || this.config.did}: code=${code}, stderr=${ffmpegStderr.join(" | ")}`);
      } else if (code && code !== 0 && proc.expectedStopReason) {
        this.platform.log.debug(`RTSP ffmpeg exited after expected stop for ${this.config.name || this.config.did}: reason=${proc.expectedStopReason}, code=${code}`);
      }
      this.metrics?.recordLiveStreamEnded();
      this.stateMachine?.liveStopped("homekit-live-ended");
      this.metrics?.increment("homekit_live_video_packets_total", videoPackets);
      this.metrics?.increment("homekit_live_audio_packets_total", audioPackets);
      this.platform.log.info(`RTSP stream ended for ${this.config.name || this.config.did}: session=${request.sessionID}, code=${code}, signal=${signal}, videoPackets=${videoPackets}, audioPackets=${audioPackets}`);
      this.clearStreamWatchdog(request.sessionID);
      this.sessions.delete(request.sessionID);
    });
  ...
  notifyPacketObservers(packet, context = {}) {
    ...
    for (const observer of Array.from(this.packetObservers)) {
      try {
        ...
        this.platform.log.debug(`Hikvision packet observer failed for ${this.config.name || this.config.did}: ${error.message}`);
      }
    }
  }
  ...
  releaseSharedReader(reader) {
    const state = this.sharedReaderStateForReader(reader) || this.defaultSharedReaderStateForRelease();
    if (!state) {
      return;
    }
    if (state.refs > 0) {
    ...
    state.idleTimer = setTimeout(() => {
      if (state.refs === 0 && state.reader) {
        this.platform.log.debug(`Closing idle Hikvision MISS reader for ${this.config.name || this.config.did}: quality=${state.quality || "default"}`);
  ...
  closeSharedReader(reader, reason) {
    ...
    const timer = setTimeout(() => {
      this.platform.log.debug(`Timed out waiting for Hikvision MISS stopMedia during ${reason}; closing reader.`);
      finish();
    }, stopTimeoutMs);
    timer.unref?.();
    ...
    Promise.resolve()
      ...
      .catch((error) => {
        this.platform.log.debug(`Could not stop Hikvision MISS media during ${reason}: ${error.message}`);
      })
      .finally(() => {
        clearTimeout(timer);
        finish();
  ...
  bumpStreamWatchdog(sessionId, reason) {
    ...
    const timer = setTimeout(() => {
      ...
      this.platform.log.warn(`Hikvision stream session ${sessionId} timed out after ${timeoutMs}ms without HomeKit stop; forcing shutdown.`);
    ...
    this.platform.log.debug(`Hikvision stream watchdog armed for ${sessionId}: reason=${reason}, timeoutMs=${timeoutMs}`);
  }
  ...
  terminateStreamProcess(sessionId, session, reason) {
    ...
    this.platform.log.info(`Terminating Hikvision stream process ${sessionId}: reason=${reason}`);
  ...
  async feedSnapshotFromPacket(packet, request) {
    ...
    this.snapshotPacketInFlight = this.captureStillFrameFromPackets(packets, request)
      ...
      .catch((error) => {
        this.platform.log.debug(`Could not capture local Hikvision snapshot from buffered live packets: ${error.message}`);
      })
      .finally(() => {
  ...
  rtspStreamUrl(purpose = "live") {
    ...
    const host = this.config.ip || this.config.host || this.config.ipAddress || this.config.address;
    if (!host || !this.config.username || !this.config.password) {
      return null;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const scheduleInputClose = () => {
        ...
        inputCloseTimer = setTimeout(() => {
          inputClosed = true;
          this.platform.log.debug(`Closing Hikvision MISS snapshot ffmpeg input for ${this.config.name || this.config.did}`);
          try {
            proc.stdio[3]?.end();
...
class AudioPacer {
  constructor(pipe, sampleRate, options = {}) {
    ...
    this.name = options.name || "Hikvision Camera";
    this.queue = [];
    this.timer = null;
    this.started = false;
    this.closed = false;
  ...
  trimBacklog() {
    ...
    while (this.queue.length > 1 && this.bufferedMs > this.maxBufferedMs) {
      ...
      if (this.droppedPackets === 1 || this.droppedPackets % 25 === 0) {
        this.log?.debug?.(`Dropped delayed Hikvision audio packets for ${this.name}: dropped=${this.droppedPackets}, bufferedMs=${this.bufferedMs}`);
      }
    }
  }
  ...
  flushNext() {
    ...
    if (this.pipe && !this.pipe.destroyed && this.pipe.writable) {
      try {
        ...
        this.log?.debug?.(`Could not write paced Hikvision audio packet for ${this.name}: ${error.message}`);
      }
    }
...
module.exports = { HikvisionCameraStreamingDelegate };
