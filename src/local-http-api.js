...
class LocalHttpApi {
  constructor(platform, config, streamingDelegate, recordingDelegate, metrics, talkback, stateMachine, monitoringService, mainPrebufferService, motionTrigger, doorbellTrigger) {
    ...
    this.doorbellTrigger = typeof doorbellTrigger === "function" ? doorbellTrigger : null;
    this.server = null;
  }
  ...
  start() {
    ...
    const host = this.config.localHttpHost || "0.0.0.0";
  ...
  async handleRequest(request, response) {
    ...
    if (url.pathname === "/api/v1/doorbell") {
      if (request.method !== "POST") {
        this.writeJson(response, 405, { ok: false, error: "method-not-allowed", method: "POST" });
        return;
      }
      ...
      this.handleDoorbellTrigger(url, response);
      return;
    }
    ...
    this.writeJson(response, 404, {
      ...
      endpoints: [
        ...
        "/api/v1/motion",
        "/api/v1/doorbell",
        "/api/v1/stream/main.h264",
        "/api/v1/stream/sub.h264",
        "/metrics",
      ],
    });
  }
  ...
  handleDoorbellTrigger(url, response) {
    if (this.config.localDoorbellTrigger !== true) {
      this.writeJson(response, 403, {
        ok: false,
        error: "local-doorbell-trigger-disabled",
      });
      return;
    }
    ...
    if (!this.doorbellTrigger) {
      this.writeJson(response, 409, {
        ok: false,
        error: "doorbell-trigger-not-available",
      });
      return;
    }

    const reason = (url.searchParams.get("reason") || "local-doorbell-trigger").trim();
    const result = this.doorbellTrigger({ source: "local-http-api", reason });
    this.writeJson(response, result.ok ? 202 : 409, {
      ...result,
      camera: this.safeCameraInfo(),
      reason,
  ...
  statusPayload() {
    ...
    return {
      ...
      doorbell: {
        localHttpEnabled: this.config.localDoorbellTrigger === true,
        available: Boolean(this.doorbellTrigger),
      },
      motionTrigger: {
  ...
  healthPayload() {
    ...
    if (protectedLocalHttp.length && !hasLocalHttpToken(this.config) && !isLoopbackHost(this.config.localHttpHost || "0.0.0.0")) {
      checks.push({
        name: "local-http-protected-auth",
        ok: false,
        detail: `Protected local HTTP endpoints enabled without localHttpToken on host ${this.config.localHttpHost || "0.0.0.0"}: ${protectedLocalHttp.join(", ")}.`,
      });
...
function protectedLocalHttpEndpoints(config) {
  ...
  if (config.localDoorbellTrigger === true) {
    endpoints.push("POST /api/v1/doorbell");
  }
